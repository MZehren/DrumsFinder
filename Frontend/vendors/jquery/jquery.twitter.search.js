...
;(function($) {
	$.fn.twitterSearch = function(options) {
		...
		return this.each(function() {
			...
			$close = $('<div class="twitterSearchClose"></div>').appendTo($frame);
			cont = $cont[0];
	...
	$.fn.twitterSearch.formatter = function(json, opts) {
		...
		str += '<div><span class="twitterSearchUser"><a class="label label-info" href="http://www.twitter.com/'+ json.from_user+'/status/'+ json.id_str +'">' 
		  + json.from_user + '</a></span>';
		pretty = prettyDate(json.created_at);
		if (opts.time && pretty)
			/*str += ' <span class="twitterSearchTime">('+ pretty +')</span>'*/
			str += ' <span class="label label-important">'+ pretty +'</span>';
